...
const normalizeTable = (table) => {
    if (!allowedTables.has(table)) {
        throw new Error(`Tabel "${table}" tidak diizinkan.`);
    }
    return table;
};
...
const buildInsertSql = (table, row) => {
    ...
    if (columns.length === 0) {
        throw new Error("Data baris kosong tidak diizinkan.");
    }
    ...
    return {
      ...
      RETURNING *
    `,
        values: columns.map((key) => normalizedRow[key]),
    };
};
const buildUpdateSql = (table, row) => {
    const id = row.id;
    if (!id) {
        throw new Error("ID wajib disertakan.");
    }
...
export default {
    async fetch(request, env) {
        ...
        if (segments.length === 0) {
            return jsonResponse({
                success: true,
                message: "Cloudflare D1 Worker API aktif.",
                database: "D1",
                version: "1.0.0",
            });
        }
        ...
        if (!table) {
            return errorResponse("Nama tabel harus disertakan.", 400);
        }
        try {
          ...
          `, [id]);
            ...
            if (request.method === "POST" &&
                ...
                const { sql, values } = buildInsertSql(table, body);
                const rows = await execute(env, sql, values);
                return jsonResponse({
                    success: true,
            ...
            if (request.method === "PUT" &&
                ...
                const { sql, values } = buildUpdateSql(table, input);
                const rows = await execute(env, sql, values);
                if (rows.length === 0) {
                    return errorResponse("Data tidak ditemukan.", 404);
                }
          ...
          `, [id]);
            ...
            if (request.method === "POST" &&
                ...
                if (!body ||
                    ...
                    body.ids.length === 0) {
                    return errorResponse("Daftar ID tidak boleh kosong.", 400);
                }
          ...
          `, ids);
            ...
            if (request.method === "POST" &&
                ...
                if (!body ||
                    ...
                    return errorResponse("Payload harus berisi array rows.", 400);
                }
            ...
            // ======================================================
            return errorResponse("Metode atau endpoint tidak didukung.", 404);
        }
        catch (error) {
            console.error("D1 Worker Error:", error);
